...
def show_reports_page():
    ...
    if user_role == "Администратор":
        ...
        m3.metric("📤 Всего ушло (себест.)", f"{total_sold_cost:,.0f} сом")
        ...
        if expense_by_day:
            ...
            for day in sorted(expense_by_day.keys()):
                rows.append({
                    "Дата": day,
                    "Ушло (шт)": expense_by_day[day]["qty"],
                    "Ушло (себест.)": round(expense_by_day[day]["cost"])
                })
            ...
            buffer = io.BytesIO()
            with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
                df_move.to_excel(writer, index=False, sheet_name="Уход со склада")
            buffer.seek(0)
            st.download_button(
                "📥 Скачать уход со склада (Excel)",
                data=buffer,
                file_name=f"Uhod_sklada_{datetime.now().strftime('%Y-%m-%d')}.xlsx",
