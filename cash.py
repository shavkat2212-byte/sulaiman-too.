...
def show_cash_page():
    ...
    if ops_res.data:
        df_ops = pd.DataFrame(ops_res.data).rename(columns={"date": "Дата и время", "amount": "Сумма (сом)", "comment": "Комментарий"})
        st.dataframe(df_ops[["Дата и время", "Сумма (сом)", "Комментарий"]], use_container_width=True, hide_index=True)
