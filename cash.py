...
# Версия: 2.1 (добавлено удаление операций + надёжная фильтрация дат)
...
def show_cash_page():
    ...
    # ==================== УДАЛЕНИЕ ОПЕРАЦИИ (ТОЛЬКО АДМИН) ====================
            ...
            if st.form_submit_button("🗑️ Удалить операцию", type="primary"):
                if confirm:
                    try:
                        ...
                        st.success("✅ Операция успешно удалена! Баланс будет пересчитан автоматически.")
                        st.rerun()
                    except Exception as e:
