...
def show_cash_page():
    ...
    # 4. Поток из таблицы cash_operations (где уже лежат все взносы и ручные операции)
    manual_cash_flow = sum(float(op['amount']) for op in ops_res.data)
    
    # ИСПРАВЛЕННАЯ ФОРМУЛА: Убран down_payments_cash, чтобы приходы не дублировались
    ...
    # Твоя оригинальная рабочая таблица истории
    st.markdown("---")
