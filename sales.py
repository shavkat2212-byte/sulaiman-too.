...
# Версия: 1.4 (правильные даты YYYY-MM-DD + предпросмотр графика)
...
def show_sales_page():
    ...
    with col_cart:
        ...
        if not st.session_state.cart:
            ...
            st.dataframe(
                cart_df[["name", "qty", "price", "Закупка (1 шт)", "total"]],
    ...
    if st.session_state.cart:
        ...
        if pay_method == "Рассрочка":
            ...
            down_payment = c_r1.number_input(
                "💵 Первоначальный взнос, сом",
                min_value=0.0,
                max_value=float(total_cart_sum),
                value=0.0
            )
            months = c_r2.number_input(
            ...
            custom_markup_percent = col_m1.number_input(
                "Процент наценки (всего за срок), %",
                min_value=0.0,
                value=float(default_markup),
                step=1.0
            )
            ...
            monthly_payment = col_m2.number_input(
                "Ежемесячный платёж (можно округлить вручную), сом",
                min_value=0,
            ...
            st.warning(
                f"📊 Расчёт: Чистый долг: {net_debt:,.0f} сом | "
                f"Всего с наценкой: {total_with_markup:,.0f} сом | "
                f"Переплата: {overpayment:,.0f} сом"
            )

            # ===== ПРЕДПРОСМОТР ГРАФИКА =====
            st.markdown("#### 📅 Предпросмотр графика платежей")
            preview_rows = []
            ...
            for i in range(1, int(months) + 1):
                ...
                preview_rows.append({
                    ...
                    "Сумма": int(amount),
                    "Остаток долга": int(max(0, balance))
                })

            st.dataframe(pd.DataFrame(preview_rows), use_container_width=True, hide_index=True)
            st.caption("Проверьте даты и суммы. Если всё верно — нажимайте кнопку ниже.")
            ...
            try:
                ...
                if pay_method == "Наличные":
                    ...
                    for i in range(1, int(months) + 1):
                        ...
                        due_str = due.strftime("%Y-%m-%d")  # ВАЖНО!
