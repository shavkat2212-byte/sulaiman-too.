...
# Версия: 1.7 (исправлен экспорт + структура)
...
def show_clients_page():
        ...
        with col_c2:
            ...
            if c_all.data:
                ...
                with st.form("client_edit_form"):
                    ...
                    new_phone = st.text_input("Изменить телефон", value=str(client_to_update.get("phone") or ""))
                    new_address = st.text_input("Изменить адрес", value=str(client_to_update.get("address") or ""))
                    new_passport = st.text_area("Изменить паспортные данные", value=str(client_to_update.get("passport") or ""))
    ...
    with tab_installments_window:
        ...
        if not c_all.data:
            ...
            for s in all_sales:
                client_fio = next((cl["fio"] for cl in c_all.data if cl["id"] == s.get("client_id")), "Неизвестный")
                sale_payments = [p for p in all_payments if p.get("sale_id") == s["id"]]
                ...
                def get_unpaid_sort(x):
                    ...
                    try:
                        return datetime.strptime(p_d, "%d.%m.%Y")
                    except:
                        return datetime.now()
                ...
                if current_debt_left > 0:
                    installments_summary.append({
                        "Клиент": client_fio,
                        "Договор / Состав товаров": s.get("name", ""),
            ...
            if selected_debtor_fio != "-- Выберите ФИО --":
                ...
                chosen_cl_sales = [s for s in all_sales if s.get("client_id") == chosen_client_id]
                ...
                if not chosen_cl_sales:
                    ...
                    for idx, s in enumerate(chosen_cl_sales):
                        details_list.append({
                            ...
                            "Дата": s.get("date", ""),
                            "Договор": s.get("name", ""),
                            "Цена продажи": int(s.get("total_sale", 0) or 0),
                            "Перв. взнос": int(s.get("down_payment", 0) or 0),
                            "Долг + наценка": int(s.get("credit_balance", 0) or 0)
                        })
                    st.table(pd.DataFrame(details_list))

                    # ----- График платежей -----
                    st.markdown("#### 🗓️ Календарный график платежей")
                    client_payments = [p for p in all_payments if p.get("client_id") == chosen_client_id]
                    ...
                    if user_role == "Администратор":
                        ...
                        sale_opts = {
                            f"{s.get('date', '')} | {str(s.get('name', ''))[:40]} | {int(s.get('total_sale', 0) or 0):,} сом": s
            ...
            if st.button("📅 Пересоздать график платежей", type="primary"):
                try:
                    ...
                    sale_options = {
                        f"{s.get('date', '')} | {str(s.get('name', ''))[:50]} | {int(s.get('total_sale', 0) or 0):,} сом": s
                        for s in chosen_cl_sales
                    }
    ...
    # =========================================================================
    # ЭКСПОРТ ВСЕХ КЛИЕНТОВ С СУММАМИ (всегда виден)
    # =========================================================================
    st.markdown("---")
