...
# Версия: 1.4 (добавлена генерация договора)
    ...
    with tab_installments_window:
        ...
        if not c_all.data:
            ...
            if not installments_summary:
                ...
                st.dataframe(
                    df_summary[[
                        "Клиент", "Договор / Состав товаров", "Закупка (сом)",
                        "Цена продажи (сом)", "Перв. взнос (сом)", "Долг + наценка (сом)",
                        "Остаток долга (сом)", "Ежемес. платёж (сом)", "Чистая прибыль (сом)"
                    ]], 
                    use_container_width=True, 
                    hide_index=True
                )
            ...
            with tab_fact:
                if st.button("🔍 Рассчитать фактические погашения", use_container_width=True):
                    try:
                        ...
                        st.error(f"Ошибка кассового отчета: {e}")
            ...
            # =====================================================
            # КАРТОЧКА КЛИЕНТА + ГЕНЕРАЦИЯ ДОГОВОРА
            # =====================================================
            ...
            selected_debtor_fio = st.selectbox(
                "Выберите ФИО клиента для детального просмотра:",
            ...
            if selected_debtor_fio != "-- Выберите ФИО --":
                ...
                if chosen_cl_sales:
                    ...
                    # Количество месяцев из платежей этого договора
                    sale_payments = [p for p in all_payments if p.get("sale_id") == selected_sale["id"]]
                    months_count = len(sale_payments) if sale_payments else 6
                    ...
                    if st.button("📄 Скачать договор (Word)", type="primary", use_container_width=True):
                        try:
                            ...
                            if not os.path.exists(template_path):
                                st.error("❌ Файл шаблона `contract_template.docx` не найден! Положи его в папку проекта.")
                            else:
                                ...
                                st.success("✅ Договор сформирован! Нажмите кнопку выше, чтобы скачать.")
                        except Exception as e:
